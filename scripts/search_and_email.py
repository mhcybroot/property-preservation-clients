#!/usr/bin/env python3
"""
Combined search + validate script.
Every run:
  1. Rotate state/category, run opencode search for new providers
  2. Validate unvalidated records (up to 5 per run) - send test emails
  3. Save validated.csv with status
  4. Send ONE combined HTML email with new records + validation summary
"""
...
CSV_FILE = f"{DATA_DIR}/services.csv"       # kept for backward compat
VALIDATED_CSV = f"{DATA_DIR}/validated.csv" # kept for backward compat
SERVICES_MD = f"{DATA_DIR}/services.md"
VALIDATED_MD = f"{DATA_DIR}/validated.md"
...
CATEGORIES = [
    ...
    "Plumber","Electrician","Lawn Care Services","Landscapper","Roofer","Inspection Crew"
]
...
def ensure_md_header(path, cols):
    if not os.path.exists(path) or os.path.getsize(path) == 0:
        with open(path, 'w') as f:
            f.write("| " + " | ".join(cols) + " |\n")
            f.write("|" + "|".join(["------"]*len(cols)) + "|\n")

def append_to_md(rows, path):
    ensure_md_header(path, ["name","email","phone","address","city","county","state","zip","category","found_date"])
    with open(path, 'a') as f:
        for r in rows:
            f.write("| " + " | ".join(str(r.get(c,'')) for c in ["name","email","phone","address","city","county","state","zip","category","found_date"]) + " |\n")
...
def save_validated_md(all_records):
    """Save validated records to both CSV (compat) and validated.md (primary)."""
    # CSV compat
    ...
    for r in all_records:
        row = {k: r.get(k, '') for k in ["name","email","phone","address","city","county","state","zip","category","found_date","email_valid","phone_valid","address_valid","verified","verified_date","notes"]}
        validated_rows.append(row)
    ensure_md_header(VALIDATED_MD, list(validated_rows[0].keys()) if validated_rows else ["name","email","phone","address","city","county","state","zip","category","found_date","email_valid","phone_valid","address_valid","verified","verified_date","notes"])
    # Overwrite MD with all validated records
    cols = ["name","email","phone","address","city","county","state","zip","category","found_date","email_valid","phone_valid","address_valid","verified","verified_date","notes"]
    with open(VALIDATED_MD, 'w') as f:
        f.write("| " + " | ".join(cols) + " |\n")
        f.write("|" + "|".join(["------"]*len(cols)) + "|\n")
...
def create_crm_lead(name, email, phone, website, address, city, state, zipcode, category):
    ...
    notes = f"Category: {category} | Address: {address}, {city}, {state} {zipcode}"[:2000]
...
def parse_table_row(row_text):
    ...
    for i, cell in enumerate(cells):
        ...
        if not address and any(x in cell_lower for x in ('ave', 'blvd', 'st ', 'dr ', 'rd ', 'tr ', 'ln ', 'way', 'pl ')):
            if len(cell) > 5:
                address = cell
...
def parse_bulleted_entry(entry_text):
    ...
    # Name from header line: **N. Business Name**
...
def parse_multi_output(output, state, city, category):
    """Parse concatenated mmx search JSON outputs (multiple JSON blobs) into rows."""
    rows = []
    ...
    import io, re
    buf = output.strip()
    # Split on JSON array/object boundaries — find each {"organic":...} block
    # Strategy: find each top-level {"organic": pattern
    chunks = re.split(r'(?=\{"organic":)', buf)
    ...
    for item in all_results:
        ...
        if is_junk_email(email) or email in seen:
            continue
        ...
        address = ""
        addr_m = re.search(
            r'\d+\s+[A-Za-z][A-Za-z\s]+?(?:ave|blvd|st|rd|dr|tr|ln|way|pl|court|ct)[.,]?\s*[A-Za-z]*\s*\d{5}',
            snippet, re.I
        )
        if addr_m:
...
def validate_all_records(max_per_run=5):
    ...
    save_validated_md(list(validated.values()))
...
def run(skip_email=False, email_only=False):
    ...
    # Multi-source query: web + social media (LinkedIn, Facebook, Instagram)
    query = (
    ...
    new_rows = parse_multi_output(output, state, city, category)
    print(f"[NEW RECORDS] {len(new_rows)}")

    crm_created = 0
    if new_rows:
        append_to_csv(new_rows, CSV_FILE)        # legacy CSV (kept for compat)
        append_to_md(new_rows, SERVICES_MD)        # primary .md storage
        add_seen_emails([r["email"] for r in new_rows])
        # CRM lead creation removed — only validated records should be pushed
    # Previously: new rows were pushed to CRM immediately (unvalidated)

    # 2. Validate unvalidated records (5 per run)
    validated_all, validated_count, auth_error = validate_all_records(max_per_run=5)

    # 3. Build and send combined HTML report
