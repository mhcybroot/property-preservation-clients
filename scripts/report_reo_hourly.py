#!/usr/bin/env python3
"""
Hourly reo_services.md report — reads, verifies, calculates, sends HTML email.
"""
...
LOG_FILE  = "/home/mhcybroot/jobs/property-preservation-clients/data/.report_reo.log"
...
def build_html(rows, stats, report_type="REO Services"):
    ...
    <div style="display:flex;gap:16px;margin-bottom:20px;">
      <div style="flex:1;background:#f3e8ff;border-radius:8px;padding:14px;text-align:center;">
        <div style="font-size:28px;font-weight:bold;color:#9334e6;">{s['total']}</div>
        <div style="font-size:12px;color:#555;">Total REO Records</div>
      </div>
      ...
      <div style="flex:1;background:#e8f0fe;border-radius:8px;padding:14px;text-align:center;">
        <div style="font-size:28px;font-weight:bold;color:#1a73e8;">{s['with_email']}</div>
    ...
    <div style="display:flex;gap:20px;margin-top:20px;">
      <div style="flex:1;">
        <h4 style="color:#9334e6;margin-bottom:8px;">Categories</h4>
        <table style="width:100%;border-collapse:collapse;font-size:12px;">{cat_rows}</table>
      </div>
      <div style="flex:1;">
        <h4 style="color:#9334e6;margin-bottom:8px;">Top States</h4>
    ...
    for r in rows[:200]:
        ...
        row_bg = "#f3e8ff" if biz else "#fff"
        email_display = email if email else "<span style='color:#ccc;'>—</span>"
        email_style = "font-weight:bold;color:#9334e6;" if biz else "color:#999;"
    ...
    table_html = f"""
    <h3 style="color:#9334e6;margin-top:20px;">Full REO Record List ({len(rows)} total, showing first 200)</h3>
    <table style="border-collapse:collapse;width:100%;font-size:12px;">
      <thead>
        <tr style="background:#9334e6;color:#fff;">
    ...
    <h1 style="color:#9334e6;border-bottom:2px solid #9334e6;padding-bottom:8px;">
    ...
    <p style="font-size:10px;color:#999;margin-top:20px;">
      Source: {REO_MD}<br/>
      Skylink LTD REO Property Preservation Pipeline
    </p>
    </body></html>"""
    return html
...
if __name__ == "__main__":
    rows = read_md_table(REO_MD)
    stats = calculate_stats(rows)
    html  = build_html(rows, stats, "REO Services Pipeline")
    subject = f"REO Services Report — {datetime.now().strftime('%Y-%m-%d %H:%M')} UTC"
    try:
